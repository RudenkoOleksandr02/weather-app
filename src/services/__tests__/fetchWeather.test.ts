...
describe('fetchWeather', () => {
    ...
    const mockedFetchCityCoordinates = fetchCityCoordinates as jest.MockedFunction<typeof fetchCityCoordinates>;
    ...
    test('Має повернути масив даних про погоду при успішному запиті', async () => {
        ...
        expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('/data/3.0/onecall'), {
    ...
    test('Має викинути помилку, якщо координати міста не знайдені', async () => {
        ...
        await expect(fetchWeather(city)).rejects.toThrow(`Місто ${city} не знайдено`);
        expect(mockedFetchCityCoordinates).toHaveBeenCalledWith(city);
